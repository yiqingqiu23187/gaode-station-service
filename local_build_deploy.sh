...
# 高德地图服务站点查询工具 - 本地构建部署脚本
# 功能：本地构建Docker镜像，推送到服务器并启动服务
...
services:
  gaode-station-service:
    ...
    environment:
      ...
      - FASTMCP_PORT=17263
    healthcheck:
      test: ["CMD", "bash", "-lc", "(pgrep -f 'python.*mcp_server.py' && pgrep -f 'python.*web_server.py') > /dev/null"]
      interval: 30s
      timeout: 10s
      retries: 3
      start_period: 40s
...
IMAGE_TAR="gaode-station-service-latest.tar"
